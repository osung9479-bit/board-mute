...
Board Mute는 지원하는 한국 커뮤니티 게시판 목록과 채용 목록에서 원치 않는 row를 로컬 규칙으로 숨기는 Chrome 확장 프로그램입니다.

현재는 Chrome Web Store 배포 전의 unpacked 확장 기준입니다. 모든 커뮤니티나 채용 사이트를 자동으로 지원하는 도구가 아니라, 아래 지원 범위에서만 동작하도록 좁게 만들었습니다.

popup은 지원 사이트 탭에서 열었을 때 해당 사이트의 규칙 화면을 자동으로 먼저 보여줍니다. 별도 `tabs` 권한을 추가하지 않고, 이미 주입된 content script와의 메시지로 현재 사이트를 확인합니다.
...
| 인벤 | PC 게시판 목록 | 닉네임, 글 ID `post:<boardId>/<postId>` |
| 네이트판 | PC 톡톡 목록 | 작성자 검색값, 글 ID `post:<postId>` |
| 사람인 | PC 검색 결과, 직무별 목록 | 회사 ID `csn:<token>`, 회사명, 공고 ID `rec:<rec_idx>` |
| 잡코리아 | PC 채용 목록 | 회사 ID `company:<token>`, 회사명, 공고 ID `gno:<gno>` |
...
- 모든 한국 커뮤니티/채용 사이트 자동 지원
- 댓글 영역 차단
- 모바일 웹
- 오늘의유머 글 보기 본문 작성자를 제외한 글 본문 작성자 차단
- 전역 작성자, 아이디, IP, 회사, 공고값 차단
- 지원 목록 row 구조가 없는 페이지의 별도 고정 관리 버튼
...
- 인벤: 공지 row, 글 보기 본문 작성자, 댓글 작성자, 베스트/화제 집계, 뉴스/웹진 기사 목록, 검색 결과, 로그인 후 개인화 영역, 모바일 웹
- 네이트판: 글 보기 본문 작성자, 댓글 작성자, 사이드바 오늘의 톡/엔터톡/베스트 톡톡, 검색 결과, 모바일 웹, 로그인 후 개인화 영역
- 사람인: 지역별 목록, 테마 채용관, 신입·인턴 홈, 상세 페이지, 모바일 웹, 로그인 후 개인화 영역
- 잡코리아: 통합 검색 결과, 공고 상세 페이지, 기업 상세/리뷰/연봉 페이지, 알바몬 공고, 로그인 후 개인화 영역, 모바일 웹
...
- `사이트` 선택으로 디시인사이드, 에펨코리아, 루리웹, 엠팍, 클리앙, 오늘의유머, 퀘이사존, SLR클럽, 인벤, 네이트판, 사람인, 잡코리아 규칙을 전환합니다.
- 사이트별 `차단` 토글을 끄면 저장된 규칙은 보존하고 해당 사이트 숨김만 멈춥니다.
- `제목 키워드`는 선택한 사이트에만 적용됩니다.
- `전역 제목 키워드`는 지원 사이트 전체에 적용됩니다.
- `작성자/아이디/IP`에는 직접 차단할 작성자 값을 추가하거나 삭제할 수 있습니다. 이 규칙은 선택한 사이트에만 적용됩니다. 사람인과 잡코리아에서는 같은 저장 영역을 `회사/공고 숨김값`으로 표시합니다.
...
숨겨진 row가 있을 때는 페이지에 `잠시 보기` / `다시 숨김` 컨트롤이 표시됩니다. 최근 페이지에서 추가한 값은 `최근 차단 관리` 또는 채용 사이트의 `최근 숨김 관리`에서 해제할 수 있습니다.
...
- content script match는 지원 사이트 12개 범위로 제한
...
```powershell
node scripts/check-supported-sites.js
...
node scripts/check-inven.js
node scripts/check-natepann.js
node scripts/check-saramin.js
node scripts/check-jobkorea.js
git diff --check
```

묶음 실행에서 하위 smoke/check의 전체 로그가 필요하면 `node scripts/check-supported-sites.js --verbose`를 사용합니다.

디시인사이드, 에펨코리아, 루리웹, 엠팍, 클리앙, 오늘의유머는 자동 검증과 실제 Chrome 최소 수동 확인을 통과한 상태입니다. 퀘이사존은 사용자 수동 확인 중 발견된 최근 차단 해제 버튼 UI를 보완했고 Chrome/CDP 재확인을 통과했습니다. SLR클럽도 현재 소스 기준 Chrome/CDP 최소 확인을 통과했습니다. 인벤은 PC 게시판 목록에서 정식 smoke로 제목 키워드, 닉네임 fallback, 특정 글, disabled site 동작을 확인했습니다. 네이트판은 PC 톡톡 목록에서 정식 smoke로 제목 키워드, 작성자 검색값 fallback, 특정 글, disabled site 동작을 확인했습니다. 사람인은 검색 결과와 직무별 목록에서 정식 smoke로 공고/조건 키워드, 회사 ID, 회사명 fallback, 특정 공고, disabled site 동작을 확인했습니다. 잡코리아는 PC 채용 목록에서 정식 smoke로 공고/조건 키워드, 회사 ID, 회사명 fallback, 특정 공고, disabled site 동작을 확인했습니다. 사이트 DOM이 바뀌면 adapter와 검증 기준을 다시 확인해야 합니다.
...
- 현재 소스 버전: `0.5.0`
- 마지막 검증 패키지: `dist/board-mute-0.5.0.zip`
- `0.5.0` SHA-256: `61B2E552AFF44B949FBEA43ABE3CD5C33A50E05EE3EDD702803276FB7C443106`
- `0.5.0` ZIP 크기: `42,184` bytes
- `0.5.0` ZIP은 잡코리아, 인벤, 네이트판을 포함한 12개 사이트 기준 제출 후보입니다.
- 남은 작업: 필요 시 public repository 커밋과 push, Chrome Web Store 제출, 추가 사이트 확장과 제품 품질 개선
